...
class ProductAttribute(models.Model):
    """Stores the type of option (e.g., 'Size', 'Color')"""
    name = models.CharField(max_length=30) 


class ProductAttributeValue(models.Model):
    """Stores the specific value (e.g., 'Small', 'Blue')"""
    attribute = models.ForeignKey(ProductAttribute, on_delete=models.CASCADE, related_name='values')
    value = models.CharField(max_length=50)


class ProductVariant(models.Model):
    """The actual item with a specific price (e.g., Jeans - Large)"""
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='variants')
    attributes = models.ManyToManyField(ProductAttributeValue)
    price = models.DecimalField(max_digits=10, decimal_places=2)  # Specific price for this size
